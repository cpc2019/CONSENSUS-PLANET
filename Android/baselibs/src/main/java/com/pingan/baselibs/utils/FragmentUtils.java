...
/**
 * Created by duyuan on 2016/12/9.
 */
...
public class FragmentUtils {
	...
	public static Fragment switchFragment(Context context, FragmentManager fragmentManager,
		Fragment currentFragment, int containerId, Fragment fragment, int index) {
		return switchFragment(context, fragmentManager, currentFragment, containerId, fragment,
			index, null);
	}

	public static Fragment switchFragment(Context context, FragmentManager fragmentManager,
		Fragment currentFragment, int containerId, Fragment f, int index, Bundle args) {
		FragmentTransaction transaction = fragmentManager.beginTransaction();
		if (null != currentFragment) {
			transaction.hide(currentFragment);
		}
		String tag = getTag(f.getClass(), index);
		Fragment fragment = fragmentManager.findFragmentByTag(tag);
		if (null == fragment) {
			fragment = Fragment.instantiate(context, f.getClass().getName(), args);
			transaction.add(containerId, fragment, tag);
		} else {
			transaction.show(fragment);
		}
