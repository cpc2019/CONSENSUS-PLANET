...
public class RollTextView<T extends Object> extends RecyclerView {
	...
	public void refreshData(List list) {
		...
		super.setLayoutManager(new LinearLayoutManager(getContext(),
			(direction == 0 || direction == 1) ? OrientationHelper.VERTICAL
	 ...
	 * 禁止主动调用该方法
	 */
	@Override @Deprecated public final void setLayoutManager(LayoutManager layout) {
	...
	private synchronized void intervalRun() {
		try {
			...
			//检查当前滚动是否错位，错位则更改
			if (lastPosition - firstPosition + 1 != getMeasuredHeight() / getChildAt(
				0).getMeasuredHeight()) {
			...
			Log.v(getClass().getSimpleName(),
				"动画执行一次。。。" + "当前最后和最开始的位置：" + firstPosition + "   " + lastPosition);
	 ...
	 * 添加itemclick事件监听
	 */
	public RollTextView setOnItemClickListener(
	 ...
	 */
	public RollTextView setCanScroll(boolean canScrollByHand) {
